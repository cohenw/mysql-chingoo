...
//	System.out.println("actionType=" + actionType);
	...
	if (actionType.equals("CHINGOO_PAGE UPDATE")) {
		String sql = "UPDATE CHINGOO_PAGE SET title='" + title + "', param1='" + param1 +
	...
	if (actionType.equals("CHINGOO_PAGE NEW")) {
		String sql = "INSERT INTO CHINGOO_PAGE (page_id, title, param1, param2, param3) VALUES ('" + pageId + "', '" + title + "', '" + param1 +
			"', '" + param2 + "', '" + param3 + "')";
	...
	if (type.equals("delete")) {
		try {
			cn.getConnection().setReadOnly(false);
			String sql = "DELETE FROM CHINGOO_PAGE WHERE page_id='" + pageId + "'";
			Statement stmt = cn.getConnection().createStatement();
			stmt.executeUpdate(sql);
			stmt.close();
		} catch (SQLException e) {
	...
	if (type.equals("deleteSql")) {
		try {
			cn.getConnection().setReadOnly(false);
			String sql = "DELETE FROM CHINGOO_PAGE_SQL WHERE page_id='" + pageId + "' AND seq = " + seq;
			Statement stmt = cn.getConnection().createStatement();
			stmt.executeUpdate(sql);
			stmt.close();
		} catch (SQLException e) {
	...
	if (actionType.equals("CHINGOO_PAGE_SQL NEW")) {
		String sql = "INSERT INTO CHINGOO_PAGE_SQL (page_id, seq, title, indent, sql_stmt) VALUES ('" + pageId + "', " + newseq + ", '" + title +
...
//		System.out.println(sql);
	...
	if (actionType.equals("CHINGOO_PAGE_SQL UPDATE")) {
		String sql = "UPDATE CHINGOO_PAGE_SQL SET seq=" + newseq + ", title='" + title + "', indent=" + indent +
...
//		System.out.println(sql);
	...
	String sql = "SELECT * FROM CHINGOO_PAGE";
...
<html>
<head> 
	<title>CHINGOO - Edit User Defined Page</title>
...
<%
	if (pageId != null || type.equals("new") ) {
		sql = "SELECT * FROM CHINGOO_PAGE_SQL WHERE PAGE_ID='" + pageId + "' ORDER BY SEQ";
		Query q2 = new Query(cn, sql);

		sql = "SELECT * FROM CHINGOO_PAGE WHERE PAGE_ID='" + pageId + "'";
...
<li>A page can have up to 3 parameters</li>
	<form method="post">
		<% if (type.equals("new")) { %>
			...
			<input type="hidden" name="actionType" value="CHINGOO_PAGE NEW">
			Page Id <input name="page" value="" size="10">
		<% } else { %>
			<h3>Edit Page "<%= pageId %>"</h3>
			<input type="hidden" name="actionType" value="CHINGOO_PAGE UPDATE">
...
<%
	if (type.equals("newSql") || type.equals("editSql")) {
		...
		sql = "SELECT * FROM CHINGOO_PAGE_SQL WHERE PAGE_ID='" + pageId + "' AND SEQ=" + seq;
		System.out.println(sql);
...
SQL: SELECT * FROM EMPLOYEE WHERE FULLNAME LIKE '%[name]%' AND CITY ='[city]';
	...
	<form method="post">
		<% if (type.equals("newSql")) { %>
			...
			<input type="hidden" name="actionType" value="CHINGOO_PAGE_SQL NEW">
			<input type="hidden" name="page" value="<%=pageId%>">
		<% } else { %>
			<h3>Edit SQL statement for "<%= pageId %>"</h3>
			<input type="hidden" name="actionType" value="CHINGOO_PAGE_SQL UPDATE">
			<input type="hidden" name="page" value="<%=pageId%>">
		<% } %>
