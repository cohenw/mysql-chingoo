...
//	System.out.println("actionType=" + actionType);
	...
	if (actionType.equals("UPDATE")) {
		String sql = "UPDATE CHINGOO_SAVED_SQL SET sql_stmt='" + Util.escapeQuote(sqlStmt) + "', timestamp=sysdate() where id='" + Util.escapeQuote(id) + "'";
System.out.println(sql);
	...
	if (actionType.equals("NEW")) {
		String sql = "INSERT INTO CHINGOO_SAVED_SQL (id, sql_stmt) VALUES ('" + Util.escapeQuote(id) + "', '" + Util.escapeQuote(sqlStmt) + "')";
	...
	if (type.equals("delete")) {
		try {
			cn.getConnection().setReadOnly(false);
			String sql = "DELETE FROM CHINGOO_SAVED_SQL WHERE id='" + Util.escapeQuote(id) + "'";
			Statement stmt = cn.getConnection().createStatement();
			stmt.executeUpdate(sql);
			stmt.close();
		} catch (SQLException e) {
	...
	String sql = "SELECT * FROM CHINGOO_SAVED_SQL";
...
<html>
<head> 
	<title>Chingoo - Edit Saved Query</title>
...
<%
	if (type.equals("new") || type.equals("edit")) {
		...
		if (id != null) {
			String s = "SELECT SQL_STMT FROM CHINGOO_SAVED_SQL WHERE ID='" + Util.escapeQuote(id) + "'";
